package tui

import (
	"testing"

	"charm.land/bubbles/v2/textarea"
	tea "charm.land/bubbletea/v2"

	"github.com/axispx/zeta/internal/ai"
	"github.com/axispx/zeta/internal/compact"
	"github.com/axispx/zeta/internal/image"
)

func histModel(prompts ...string) Model {
	ta := textarea.New()
	ta.Focus()
	m := Model{composer: composer{textarea: ta}}
	for _, p := range prompts {
		m.transcript.messages = append(m.transcript.messages, Message{Role: RoleUser, Text: p})
	}
	m.resetPromptHistory()
	return m
}

func histKey(s string) tea.KeyPressMsg {
	switch s {
	case "up":
		return tea.KeyPressMsg{Code: tea.KeyUp}
	case "down":
		return tea.KeyPressMsg{Code: tea.KeyDown}
	case "ctrl+p":
		return tea.KeyPressMsg{Code: 'p', Mod: tea.ModCtrl}
	case "ctrl+n":
		return tea.KeyPressMsg{Code: 'n', Mod: tea.ModCtrl}
	default:
		return tea.KeyPressMsg{Text: s}
	}
}

func TestStepUserMessage(t *testing.T) {
	msgs := []Message{
		{Role: RoleUser, Text: "one"},
		{Role: RoleAgent, Text: "ok"},
		{Role: RoleUser, Text: "two"},
	}
	if got := stepUserMessage(msgs, len(msgs), -1); got != 2 {
		t.Fatalf("oldest-from-live = %d", got)
	}
	if got := stepUserMessage(msgs, 2, -1); got != 0 {
		t.Fatalf("older-from-two = %d", got)
	}
	if got := stepUserMessage(msgs, 0, -1); got != -1 {
		t.Fatalf("older-at-oldest = %d", got)
	}
	if got := stepUserMessage(msgs, 0, +1); got != 2 {
		t.Fatalf("newer-from-one = %d", got)
	}
	if got := stepUserMessage(msgs, 2, +1); got != -1 {
		t.Fatalf("newer-at-newest = %d", got)
	}
}

func TestPromptHistoryEmpty(t *testing.T) {
	m := histModel()
	if m.handlePromptHistoryKey(histKey("up")) {
		t.Fatal("up with empty history should pass through")
	}
	if m.handlePromptHistoryKey(histKey("down")) {
		t.Fatal("down with empty history should pass through")
	}
}

func TestPromptHistoryNavigate(t *testing.T) {
	m := histModel("first", "second")
	// Interleave agent so indices are not dense user-only.
	m.transcript.messages = []Message{
		{Role: RoleUser, Text: "first"},
		{Role: RoleAgent, Text: "ok"},
		{Role: RoleUser, Text: "second"},
	}
	m.resetPromptHistory()
	m.composer.textarea.SetValue("draft")

	if !m.handlePromptHistoryKey(histKey("up")) {
		t.Fatal("up should be consumed")
	}
	if m.composer.textarea.Value() != "second" {
		t.Fatalf("want second, got %q", m.composer.textarea.Value())
	}
	if m.composer.promptHist.draft.Text != "draft" || m.composer.promptHist.at != 2 {
		t.Fatalf("browse = %+v", m.composer.promptHist)
	}

	if !m.handlePromptHistoryKey(histKey("ctrl+p")) {
		t.Fatal("ctrl+p should be consumed")
	}
	if m.composer.textarea.Value() != "first" || m.composer.promptHist.at != 0 {
		t.Fatalf("want first@0, got %q@%d", m.composer.textarea.Value(), m.composer.promptHist.at)
	}

	// Already at oldest: still consumed, value unchanged.
	if !m.handlePromptHistoryKey(histKey("up")) {
		t.Fatal("up at oldest should be consumed")
	}
	if m.composer.textarea.Value() != "first" {
		t.Fatalf("still first, got %q", m.composer.textarea.Value())
	}

	if !m.handlePromptHistoryKey(histKey("down")) {
		t.Fatal("down should be consumed")
	}
	if m.composer.textarea.Value() != "second" {
		t.Fatalf("want second, got %q", m.composer.textarea.Value())
	}

	if !m.handlePromptHistoryKey(histKey("ctrl+n")) {
		t.Fatal("ctrl+n should be consumed")
	}
	if m.composer.textarea.Value() != "draft" {
		t.Fatalf("want draft restored, got %q", m.composer.textarea.Value())
	}
	if !m.composer.promptHist.live() || m.composer.promptHist.draft.Text != "" {
		t.Fatalf("want live, got %+v", m.composer.promptHist)
	}
}

func TestPromptHistoryIgnoresCompactCheckpoint(t *testing.T) {
	// After compaction, API history starts with a synthetic user checkpoint.
	// Up/down must still walk real user prompts from the UI transcript only.
	m := histModel()
	cp := compact.CheckpointMessage("## Task\n- secret summary blob")
	m.session.History = []ai.Message{
		cp,
		{Role: ai.RoleUser, Text: "after compact"},
	}
	m.transcript.messages = []Message{
		{Role: RoleUser, Text: "before compact"},
		{Role: RoleAgent, Text: "ok"},
		{Role: RoleSystem, Text: compactDividerText},
		{Role: RoleUser, Text: "after compact"},
	}
	m.resetPromptHistory()
	m.composer.textarea.SetValue("draft")

	if !m.handlePromptHistoryKey(histKey("up")) {
		t.Fatal("up")
	}
	if m.composer.textarea.Value() != "after compact" {
		t.Fatalf("got %q", m.composer.textarea.Value())
	}
	if !m.handlePromptHistoryKey(histKey("up")) {
		t.Fatal("up older")
	}
	if m.composer.textarea.Value() != "before compact" {
		t.Fatalf("want pre-compact prompt, got %q", m.composer.textarea.Value())
	}
	// Must never surface the checkpoint XML.
	if m.composer.textarea.Value() == cp.Text {
		t.Fatal("checkpoint leaked into prompt history")
	}
}

func TestPromptHistoryMultilineBoundary(t *testing.T) {
	m := histModel("old")
	m.composer.textarea.SetValue("line1\nline2")
	m.composer.textarea.MoveToEnd() // last line

	// From last line, up should NOT enter history (Line() > 0).
	if m.composer.textarea.Line() == 0 {
		t.Fatal("expected cursor on last line of multiline")
	}
	if m.handlePromptHistoryKey(histKey("up")) {
		t.Fatal("up mid-draft should pass through to textarea")
	}
	if m.composer.textarea.Value() != "line1\nline2" {
		t.Fatalf("value changed: %q", m.composer.textarea.Value())
	}

	m.composer.textarea.MoveToBegin()
	if m.composer.textarea.Line() != 0 {
		t.Fatalf("Line = %d", m.composer.textarea.Line())
	}
	if !m.handlePromptHistoryKey(histKey("up")) {
		t.Fatal("up at first line should enter history")
	}
	if m.composer.textarea.Value() != "old" {
		t.Fatalf("want old, got %q", m.composer.textarea.Value())
	}
	if m.composer.promptHist.draft.Text != "line1\nline2" {
		t.Fatalf("draft = %q", m.composer.promptHist.draft.Text)
	}

	// Down from first line of a multiline recall should pass through.
	m.composer.textarea.SetValue("x\ny")
	m.composer.textarea.MoveToBegin()
	if m.handlePromptHistoryKey(histKey("down")) {
		t.Fatal("down not on last line should pass through")
	}
}

func TestPromptHistoryEditExitsBrowse(t *testing.T) {
	m := histModel("old")
	if !m.handlePromptHistoryKey(histKey("up")) {
		t.Fatal("up")
	}
	before := m.composer.textarea.Value()
	m.composer.textarea.SetValue(before + "!")
	m.notePromptEdit(before)
	if !m.composer.promptHist.live() {
		t.Fatalf("edit should exit browse, got %+v", m.composer.promptHist)
	}
	if m.composer.textarea.Value() != "old!" {
		t.Fatalf("value = %q", m.composer.textarea.Value())
	}
	// Cursor-only change does not exit.
	_ = m.handlePromptHistoryKey(histKey("up"))
	before = m.composer.textarea.Value()
	m.notePromptEdit(before)
	if m.composer.promptHist.live() {
		t.Fatal("unchanged value should stay browsing")
	}
}

func TestResetPromptHistory(t *testing.T) {
	m := histModel("a")
	m.composer.textarea.SetValue("x")
	_ = m.handlePromptHistoryKey(histKey("up"))
	m.resetInput()
	if !m.composer.promptHist.live() || m.composer.promptHist.draft.Text != "" {
		t.Fatalf("prompts=%+v", m.composer.promptHist)
	}
	if m.composer.textarea.Value() != "" {
		t.Fatalf("input = %q", m.composer.textarea.Value())
	}
}

func TestPromptHistoryDraftImagesRoundTrip(t *testing.T) {
	const dataURL = "data:image/png;base64,iVBORw0KGgo="
	m := histModel("prior")
	m.insertImageAttach(image.Ref{URL: dataURL, MIME: "image/png", Name: "draft.png"})
	draftVal := m.composer.textarea.Value()

	if !m.handlePromptHistoryKey(histKey("up")) {
		t.Fatal("up should recall prior")
	}
	if m.composer.textarea.Value() != "prior" {
		t.Fatalf("recalled=%q", m.composer.textarea.Value())
	}
	if len(m.composer.pendingImages) != 0 {
		t.Fatal("pending should clear while browsing text-only entry")
	}
	if !m.handlePromptHistoryKey(histKey("down")) {
		t.Fatal("down should restore draft")
	}
	if m.composer.textarea.Value() != draftVal {
		t.Fatalf("restored draft=%q want %q", m.composer.textarea.Value(), draftVal)
	}
	if len(m.composer.pendingImages) != 1 || m.composer.pendingImages[1].URL != dataURL {
		t.Fatalf("pending not restored: %+v", m.composer.pendingImages)
	}
}
